...
while True:
    if scd4x.data_ready:
        ...
        #response = lora_send_text("CO2 ppm " % scd4x.CO2)
        #response = lora_send_text(str(scd4x.CO2), False)
        co2ppm = scd4x.CO2
        temp = scd4x.temperature
        humidity = scd4x.relative_humidity
        resp = str(co2ppm) + " " + str(temp) + " " + str(humidity)         
        response = lora_send_text(resp)
        #response = lora_send_text(str(temp)) 	# ok
        #print ("response = ")
        #print (response)
        #print ("end_response")
        time.sleep(300) 
